...
part 'interval_time.freezed.dart';
part 'interval_time.g.dart';

@freezed
class IntervalTime with _$IntervalTime {
  const IntervalTime._();
  const factory IntervalTime({
    double? beta,
    @Default(0) int days,
    @Default(0) int hours,
  ...
  factory IntervalTime.fromBeta(double beta) {
    ...
    final min = (hr - hrTr) * 60;
    final minTr = min.toInt();

    final sec = (min - minTr) * 60;
    final secTr = sec.toInt();

    final millSec = (sec - secTr) * 1000;
    final millSecTr = millSec.toInt();
    ...
    return IntervalTime(
      ...
      minutes: minTr,
      seconds: secTr,
      milliseconds: millSecTr,
      microseconds: 0,
    );
  }

  factory IntervalTime.fromJson(Map<String, dynamic> json) =>
      _$IntervalTimeFromJson(json);

  double get betaValue =>
      beta ?? duration.inMilliseconds / Duration.millisecondsPerDay;

  String get valueMinuteSecond => '${_format(minutes)}:${_format(seconds)}';

  String get valueMinuteSecondMillisecondTwo =>
      '${_format(minutes)}:${_format(seconds)}:$milliseconds';
  ...
  String get valueMinuteSecondMillisecondOne {
    ...
    if (milliseconds.toString().length == 1) {
      ...
      milliFormat = '${milliseconds ~/ 10}';
    } else {
      milliFormat = '${milliseconds ~/ 100}';
    }

    return '${_format(minutes)}:${_format(seconds)}:$milliFormat';
  }

  String _format(int value) => value < 10 ? '0$value' : '$value';
}
